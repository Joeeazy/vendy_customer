      ...
      <div className="mt-3 flex flex-col gap-5">
        {services.map((service) => (
          <div key={service.service_type_id}>
            {grouped && (
              <h3 className="mb-1 text-caption font-semibold tracking-wide text-slate uppercase">
                {service.name}
              </h3>
            )}
                ...
                <li key={item.id} className="flex items-baseline justify-between gap-4 py-3 hairline-b">
                  <span>
                    ...
                    {item.warranty_days > 0 && (
                      <span className="block text-caption text-slate">{item.warranty_days}-day warranty</span>
                    )}
                  </span>
                  <span className="text-right font-display font-bold whitespace-nowrap tabular">
                    {priceLabel(item)}
                  </span>
                </li>
              ))}
